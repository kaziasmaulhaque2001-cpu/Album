...
import { X, Calendar, User, Heart, Lock, CalendarOff, AlertCircle, ToggleLeft, ToggleRight } from "lucide-react";
...
export default function EditAlbumModal({ isOpen, onClose, onSuccess, album }: EditAlbumModalProps) {
  ...
  useEffect(() => {
    if (album) {
      setBrideName(album.brideName);
      setGroomName(album.groomName);
      
      // format date as YYYY-MM-DD
      const dateObj = new Date(album.weddingDate);
      ...
      if (album.expiryDate) {
        const expObj = new Date(album.expiryDate);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Failed to update wedding album.");
      }
          ...
          >
            ...
            <div className="bg-[#121211] p-6 text-white relative">
              ...
              <span className="text-[#D4AF37] font-mono text-[9px] tracking-[0.3em] uppercase block mb-1">
                Bespoke Creator Suite
              </span>
              <h2 className="text-2xl font-serif font-light tracking-wide flex items-center gap-2">
                Configure Album <Heart className="h-5 w-5 text-[#D4AF37] fill-[#D4AF37]/20" />
              </h2>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-6 max-h-[75vh] overflow-y-auto">
              ...
              {/* Status Toggle */}
              <div className="flex items-center justify-between p-4 bg-neutral-50 rounded-xl border border-neutral-150">
                <div>
                  <span className="block text-xs font-semibold text-neutral-900">
                    Gallery Link Activation Status
                  </span>
                  <p className="text-[11px] text-neutral-500 font-light mt-0.5">
                    Toggle to disable or enable the unique customer portal link instantly.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => setIsActive(!isActive)}
                  className="text-neutral-700 transition-colors cursor-pointer focus:outline-none"
                >
                  {isActive ? (
                    ...
                    <div className="flex items-center gap-2 text-neutral-400 font-mono text-xs font-semibold">
                      <span>DISABLED</span>
                      <ToggleLeft className="h-9 w-9 text-neutral-300" />
                    </div>
                  )}
                </button>
              </div>
